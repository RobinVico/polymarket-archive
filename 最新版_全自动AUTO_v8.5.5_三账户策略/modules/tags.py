...
TAGS = {
    ...
    # "Politics" 已删除 (2026-07-12 用户令): 智谱内容过滤每一轮都拦它 (400 contentFilter, 实测 3/3),
    # 白白报错+浪费扫描。US Politics (范围窄) 保留。要恢复就把条目加回来。
    "US Politics": {
        "tier": 2,
...
# AUTO 分叉 (2026-07-12 用户拍板, 从"关键词黑名单永远排除"改为"永远只进测试仓"):
# 白名单 tag 扫描里自然出现的黑名单市场**不再从报告里删**, 照样喂 GLM; scanner 在这些市场的
# slug 行尾打这个标记。下游读它:
#   - auto_trader: 见标记 → 无视价格强制只进测试仓 (绝不真买), 状态记 paper:黑名单;
#   - auto_discovery: 数候选门槛时扣掉带标记的, 让黑名单绝不单独触发一次 GLM 调用 ("不特意搜黑名单")。
# 用报告标记 (基于市场**原始标题**判定) 而非事后拿 GLM 可能改写过的标题重判, 避免 "fed rate" 这类
# 带空格的词在带连字符的 slug 里漏判。改标记文案时下游全靠 import 这个常量, 不会漂移。
BLACKLIST_REPORT_MARK = "【含黑名单关键词·仅测试仓】"
...
def is_blacklisted(market_obj, event_obj=None):
    """
    判断market是否应被黑名单排除.
    优先级 (AUTO 分叉 2026-07-10, 用户拍板"关键词黑名单永远生效"):
    0. **关键词命中标题/slug → 永远排除, 白名单 tag 也救不了**
       (背景: bitcoin-GTA 市场挂着白名单 tag 被"白名单覆盖黑名单"放行, 全自动直接买成真仓。
        口径收窄到 question+slug = 市场本体就是黑名单主题; description 顺带提及不算, 防误伤
        伊朗制裁类市场描述里提到 inflation 之类。)
    1. 白名单 tag (TAGS 全部 tier 1-4) - 命中任一则放行 (仍覆盖 tag 级黑名单, 保 Olympics 案例)
    2. 检查 event 的 tags 是否含黑名单 tag
    3. 检查 market 的 question/slug/description 是否含黑名单 keyword (老行为, 无白名单 tag 时兜底)
    """
    import re as _re
    # 0. AUTO 分叉 (2026-07-10): 关键词黑名单永远生效 (只看标题+slug)
    _title_text = (market_obj.get("question", "") + " " + market_obj.get("slug", "")).lower()
    for _kw in BLACKLIST_KEYWORDS:
        if _re.search(r"\b" + _re.escape(_kw) + r"\b", _title_text):
            return True
