...
def expire_stale_auto_reeval(analyzing_min=30, executing_min=10):
    ...
    返回复位行数。
    AUTO 分叉补丁 (2026-07-10): agent 多轮深搜的重评单仓正常就要跑 30~60+ 分钟, 老的 30 分钟判死线
    会误杀活着的慢仓 (实测 2026-07-10 id=12) → env AUTO_REEVAL_STALE_ANALYZING_MIN (默认 90 分钟)
    覆盖调用方 (monitor.py 写死的 30) 传进来的值。同步老项目代码时保留本段。"""
    import os as _os
    try:
        analyzing_min = float(_os.environ.get("AUTO_REEVAL_STALE_ANALYZING_MIN", "90") or analyzing_min)
...
def get_presence():
    ...
    # AUTO 分叉补丁 (用户 2026-07-08 "全部自动"): 本实例永远按「离线」跑 —— 重评决策自动执行,
    # 打开页面看盘不暂停自动化。env AUTO_FORCE_OFFLINE=0 可恢复老的在线/离线行为。
    import os as _os
    if _os.environ.get("AUTO_FORCE_OFFLINE", "1") == "1":
        return {"online": online, "at": at, "fresh": fresh, "effective_online": False,
                "idle_sec": idle_sec, "manual_off": manual_off, "forced_offline": True}
