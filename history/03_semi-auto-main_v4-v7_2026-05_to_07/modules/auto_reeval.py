...
GLM_MAX_TOKENS = _i("AUTO_REEVAL_GLM_MAX_TOKENS", 120000)         # v7.5.2: 32K→120K (贴近 glm-5.2 128K 输出上限). reasoning_effort=max 思考极长, 32K 常在吐 JSON 前就被 finish_reason=length 截断 → "未返回 JSON 决策"; 给足预算让它想完再吐 JSON.
GLM_TIMEOUT = _f("AUTO_REEVAL_GLM_TIMEOUT", 1200)                 # v7.5.2: client 超时 600→1200s (20min, 仍 < 30min expire_stale). 120K token + 深思考更慢, 给足时间"等他写完".
    ...
    "input_schema": {
        ...
        "properties": {
            "action": {
                ...
                "description": "hold=继续持有(自动止损照旧); update_q=继续持有但更新概率估计; exit=立即/提前止损(清仓); cancel_autostop=取消这个仓的自动止损, 容忍它继续亏(只在论点没破、纯回撤、判断该扛时用; ⚠️事件型仓位禁用 cancel_autostop — 它已有很松止损+护栏, 想扛请用 hold/update_q)",
            },
            "new_q": {
                "type": "number",
...
- exit: 立即/提前止损 (现在卖出)
- cancel_autostop: 取消这个仓的自动止损, 容忍它继续亏 (只在论点没破、纯回撤、你判断该扛时用; ⚠️事件型仓位不要用此项 — 它已有很松止损+护栏, 想扛请用 hold / update_q)
...
def guard_event_driven_cancel(action, decision, tier):
    """v7.5.3 安全闸: event_driven 仓的 cancel_autostop (API 重评) 一律禁止 → 降级 update_q(有合法 new_q)或 hold。
    事件型已有很松的 -60% 止损(7.4.3)+ exit 护栏, 不该再让 API 把这道 -60% 兜底撤成只剩地板 (那正是 7.4.3 特意堵的坑)。
    想关只有用户手动 /api/toggle_auto_stop。返回 (action_eff, downgraded, why)。纯函数, 不碰 DB。
    非 cancel_autostop / 非 event_driven → 原样返回。"""
    if action != "cancel_autostop" or tier != "event_driven":
        return (action, False, "")
    nq = (decision or {}).get("new_q")
    try:
        has_nq = nq is not None and 0.0 < float(nq) < 1.0
    except Exception:
        has_nq = False
    new_action = "update_q" if has_nq else "hold"
    why = f"event_driven 护栏: API cancel_autostop 禁止 → 降级 {new_action} (保住 -60% 兜底不被 API 撤; 想关只能用户手动)"
    return (new_action, True, why)
...
- exit = 立即/提前止损 (现在卖)
- cancel_autostop = 取消这个仓的自动止损, 容忍继续亏 (仅在论点没破、纯回撤、你判断该扛时用; ⚠️事件型仓位不要用 — 已有很松止损+护栏, 想扛用 hold/update_q)
...
action 含义: hold=继续持有(止损照旧) / update_q=继续持有但更新 q / exit=立即止损清仓 / cancel_autostop=取消该仓自动止损容忍继续亏(仅论点没破、纯回撤时用; 事件型禁用, 想扛用 hold/update_q)。
...
def _parse_glm_decision(text):
    ...
    if not blob:
        return {"error": "GLM 未返回 JSON 决策", "raw_text": raw[:4000]}
    d = None
    try:
        d = _json.loads(blob)
    except Exception:
...
def _run_glm(pos, meta):
    ...
    try:
        client = ZhipuAI(timeout=GLM_TIMEOUT)   # v7.5.2: 深度思考+大输出可能慢, 给足超时 (默认 1200s, env AUTO_REEVAL_GLM_TIMEOUT)
    except TypeError:
    ...
    try:
        ...
        _fin = getattr(_choice, "finish_reason", None)
    ...
    # v7.5.2: 记 finish_reason + token 用量 → 下次一眼看出是不是被 token 上限截断 (length) 而非超时/别的
    try:
        _usage = getattr(resp, "usage", None)
        _ct = getattr(_usage, "completion_tokens", None) if _usage else None
        log.info(f"GLM 响应: finish_reason={_fin} completion_tokens={_ct}/{GLM_MAX_TOKENS} text_len={len(text)}")
        if _fin == "length":
            log.warning(f"⚠️ GLM 被 max_tokens={GLM_MAX_TOKENS} 截断 (finish_reason=length) → 可能没吐完 JSON; 再不够就调高 AUTO_REEVAL_GLM_MAX_TOKENS")
    except Exception:
        pass
...
def _auto_execute(sug_id, d, pos, meta):
    ...
    try:
        ...
        action, _dc, _wc = guard_event_driven_cancel(action, d, _tier)
        if _dc:
            log.warning(f"auto_reeval id={sug_id}: {_wc}")
        if action == "exit":
            from modules.executor import Executor
            exe = Executor.get()
