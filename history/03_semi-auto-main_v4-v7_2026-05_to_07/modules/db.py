...
def init_db():
    ...
    conn.executescript("""
        ...
        -- v7.5: ⚡自动录入 — 主页存下的 DISCOVERY 推荐, monitor 心跳拿新仓位来配, 配上自动写 meta.
        CREATE TABLE IF NOT EXISTS pending_recs (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            slug TEXT NOT NULL,
            side TEXT NOT NULL,
            q REAL,
        ...
        CREATE INDEX IF NOT EXISTS idx_pending_recs_status ON pending_recs(status);
    """)
    # v7.5.1: pending_recs 加解析字段 — 存入时把 Claude 的 slug 解析成真实市场 (token 匹配防烂 slug)
    for col, decl in [
        ("token_yes", "TEXT"),
        ("token_no", "TEXT"),
        ("resolved_slug", "TEXT"),
        ...
        try:
            conn.execute(f"ALTER TABLE pending_recs ADD COLUMN {col} {decl}")
...
# ==================== v7.5 ⚡自动录入 (pending_recs) ====================
# 主页 JSON 快速通道「⚡ 存入自动录入」把推荐存进来 → monitor 心跳拿新出现的无 meta 仓位
# 按 slug+side 配对, 配上自动写全套 position_meta。状态: pending / applied (自动录入成功) /
# superseded (被更新的同 slug+side 推荐顶掉, 或用户已手动录入) / expired (超 24h) / cleared (手动清空)。
# 「清空不删数据」与项目惯例一致。

def add_pending_recs(recs):
    """存一批推荐等自动录入。同 slug+side 已有 pending 的旧条目标 superseded (新的顶掉旧的)。
    返回通过校验存入的条数。校验: slug 非空 + side∈YES/NO + q∈(0,1); tier 非法则置空 (兜底当 hybrid)。"""
    conn = get_conn()
    n = 0
    now = _utc_now_iso()
    for r in recs or []:
        ...
        try:
            ...
            continue
        if not slug or side not in ("YES", "NO") or not (0 < q < 1):
            continue
        tier = (r.get("stop_loss_tier") or "").strip() or None
        if tier and tier not in ("convergent", "hybrid", "event_driven"):
            tier = None
        cluster = (r.get("cluster_id") or "").strip() or None
        if cluster and "-" not in cluster:
            cluster = None  # 与 /api/record_position 同款格式要求 (topic-direction)
        try:
            cur_price = float(r["cur_price"]) if r.get("cur_price") not in (None, "") else None
        except (TypeError, ValueError):
            cur_price = None
        resolved_slug = (r.get("resolved_slug") or "").strip() or None
        conn.execute("""UPDATE pending_recs SET status='superseded' WHERE side=? AND status='pending'
                        AND (slug=? OR (? IS NOT NULL AND (slug=? OR resolved_slug=?)))""",
                     (side, slug, resolved_slug, resolved_slug, resolved_slug))
        conn.execute("""INSERT INTO pending_recs
            (slug, side, q, confidence, stop_loss_tier, cluster_id, tag, end_date, cur_price, reason,
             token_yes, token_no, resolved_slug, resolved_title, status, created_at)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,'pending',?)""",
                     (slug, side, q,
                      (r.get("confidence") or "").strip().lower() or None,
                      tier, cluster,
                      (r.get("tag") or "").strip() or None,
                      (r.get("end_date") or "").strip() or None,
                      cur_price,
                      (r.get("reason") or "").strip()[:2000] or None,
                      (r.get("token_yes") or "").strip() or None,
                      (r.get("token_no") or "").strip() or None,
                      resolved_slug,
                      (r.get("resolved_title") or "").strip() or None,
                      now))
        n += 1
    conn.commit(); conn.close()
    return n
...
def expire_pending_recs(ttl_h=24):
    """超过 ttl (默认 24h = 用户要的"存一天") 的 pending 标 expired。返回过期条数。"""
    conn = get_conn()
    cutoff = (datetime.now(timezone.utc) - timedelta(hours=ttl_h)).isoformat()
    cur = conn.execute("UPDATE pending_recs SET status='expired' WHERE status='pending' AND created_at < ?",
                       (cutoff,))
    conn.commit(); n = cur.rowcount; conn.close()
    return n


def get_pending_recs(status="pending"):
    """status=None 返回最近 100 条全状态 (给前端状态栏); 传 status 只返回该状态。"""
    conn = get_conn()
    if status:
        rows = conn.execute("SELECT * FROM pending_recs WHERE status=? ORDER BY id DESC", (status,)).fetchall()
    else:
        rows = conn.execute("SELECT * FROM pending_recs ORDER BY id DESC LIMIT 100").fetchall()
    conn.close()
    return [dict(r) for r in rows]


def mark_pending_rec(rec_id, status, token_id=None):
    conn = get_conn()
    conn.execute("UPDATE pending_recs SET status=?, applied_at=?, applied_token_id=? WHERE id=?",
                 (status, _utc_now_iso(), token_id, rec_id))
    conn.commit(); conn.close()


def clear_pending_recs():
    """清空所有待录 (标 cleared 不删数据)。返回清掉的条数。"""
    conn = get_conn()
    cur = conn.execute("UPDATE pending_recs SET status='cleared' WHERE status='pending'")
    conn.commit(); n = cur.rowcount; conn.close()
    return n
