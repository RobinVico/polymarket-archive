...
内容 = v7.5.4 现行策略快照 (2026-07-08 从代码核对):
...
输出: 项目根目录 Polymarket_Bot_策略总览_v7.5.4.pptx
"""
import os
...
callout(s, ML, Inches(4.14), CW, Inches(0.95),
        ...
        '收敛型 / 事件型: 不马上卖, 先进「⏸ 等重评」让 AI 联网重查再决定 (P7–P8)。混合型: 到值直接平仓 (不走重评, 因它扛输最差)。任何仓 ① 跌破 $0.05 地板 ② 重评没配置 也直接卖。',
        bar=RED, bg=RED_BG)
...
bullets(s, ML, Inches(1.66), Inches(6.15), [
    ...
    {'m': '③', 'mc': GOLD, 'b': '止损 / 移动止损', 't': ' (P5) — 收敛/事件型进「⏸ 等重评」, 混合型直接平仓'},
...
bullets(s, ML, Inches(5.55), CW, [
    ...
    ('⚡ 自动录入 (v7.5.0): ', '推荐点「⚡ 存入自动录入」存服务器 → 你去 Polymarket 买完, 新仓一出现 bot 心跳就按市场自动配对、写全 q/信心/止损档/簇 (跟手动录同口径, 入场价用实时均价); 配不上 / TP方向反了 → 主页琥珀闪窗提醒你手动补。只写元数据, 绝不下单'),
], size=12.2, gap=8)
