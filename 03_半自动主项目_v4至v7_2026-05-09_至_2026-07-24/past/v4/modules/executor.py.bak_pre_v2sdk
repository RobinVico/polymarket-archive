...
load_dotenv("/Users/baymaxagent/polymarket-semi-auto/.env")
...
class Executor:
    def __init__(self):
        ...
        try:
            ...
            if not key:
                ...
                return
            self.client = ClobClient(host=HOST, chain_id=CHAIN_ID, key=key, signature_type=sig, funder=funder)
    ...
    def sell(self, token_id, size, reason=""):
        ...
        try:
            ...
            # === Step 1: size 2位小数 (Polymarket SELL精度限制) ===
            size = round(size, 2)
            if size < 0.01:
            ...
            # === Step 4: FAK订单 (能成交多少成交多少, 剩余cancel) ===
            order_args = OrderArgs(price=best_bid, size=size, side=SELL, token_id=token_id)
            try:
                signed = self.client.create_order(order_args)
                result = self.client.post_order(signed, OrderType.FAK)
            except Exception as e:
                log.exception(f"sell: post_order异常: {e}")
                return False
    ...
    def sell_partial(self, token_id, total_size, pct, reason=""):
        sell_size = round(total_size * pct / 100, 2)
        if sell_size < 0.1:
            sell_size = total_size
        return self.sell(token_id, sell_size, reason)
