...
<!-- v7.5 ⚡自动录入失配提醒: 琥珀色, 跟红色紧急弹窗(重评)分开, 不抢一个框 -->
<div id="autorec-pop" style="display:none;position:fixed;top:128px;left:50%;transform:translateX(-50%);z-index:9997;background:#241a06;border:2px solid #e8b93e;border-radius:10px;padding:12px 18px;box-shadow:0 0 20px rgba(232,185,62,0.5);animation:urgentflash 0.8s infinite;max-width:80vw">
<span id="autorec-pop-text" style="color:#ffd766;font-weight:700;font-size:13px"></span>
<button class="btn-small" style="margin-left:12px" onclick="arcDismiss()">知道了</button>
</div>
...
<button class="btn" style="align-self:center" onclick="cjSaveAuto()" title="v7.5: 把解析出的推荐存到服务器 24 小时 — 之后只管去 Polymarket 下单, 新仓位一出现 bot 自动把 q/信心/止损/cluster/tag 全部录入(30 秒内), 不用回来点; 对不上推荐的新仓位会闪提醒">⚡ 存入自动录入</button>
...
<select id="tier-{{ loop.index0 }}" class="conf-sel" title="止损分级 (Claude 入场分类决定): 收敛 回撤20%(≤3天12%)+确认→交重评 / 混合 回撤35%+确认→直接平仓(v7.4.5, 不走重评) / 事件 入场-60%+$0.05地板→交重评 (必须选一档, 无未分类)" onchange="saveTier('{{ p.asset }}', this.value)">
...
// v7.5 ⚡自动录入: 推荐存服务器 24h → bot 心跳自动配新仓位写全套 meta; 配不上的闪琥珀提醒。
// 状态轮询 20s: 待命/已自动录/待人工 chips + 已自动录的从草稿里自动拿掉 (同手动录入成功行为)。
function cjSaveAuto(){
  if(!cjRecs.length){ showT('err','先粘 JSON 点「✨ 解析」, 再存自动录入'); return; }
  fetch('/api/pending_recs',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({recs:cjRecs})})
    .then(r=>r.json()).then(d=>{
      if(!d.ok){ showT('err', d.message||'存入失败'); return; }
      let msg='⚡ 已存 '+d.saved+' 条 (24h): 去下单即可, 仓位出现后 30 秒内自动录全套';
      if(d.resolved && d.resolved.length) msg+=' · 已解析市场: '+d.resolved.map(x=>x.title).slice(0,3).join(' / ');
      if(d.unresolved && d.unresolved.length) msg+=' · ⚠ '+d.unresolved.length+' 条 slug 查不到市场 (会按相似度兜底匹配)';
      showT('ok', msg);
      cjAutoLoad();
    }).catch(()=>showT('err','存入失败 (网络)'));
}
function arcDismiss(){
  try{ localStorage.setItem('pm_autorec_dismiss', (document.getElementById('autorec-pop-text')||{}).textContent||''); }catch(_){}
  document.getElementById('autorec-pop').style.display='none';
}
function cjAutoClear(){
  fetch('/api/pending_recs/clear',{method:'POST'}).then(r=>r.json()).then(d=>{
    showT('ok','已停用: 清空待录 '+(d.cleared||0)+' 条'); cjAutoLoad();
  }).catch(()=>{});
}
function cjAutoLoad(){
  fetch('/api/pending_recs').then(r=>r.json()).then(d=>{
    if(!d || !d.ok) return;
    const box = document.getElementById('cj-auto');
    const pend = (d.recs||[]).filter(r=>r.status==='pending');
    const applied = (d.recs||[]).filter(r=>r.status==='applied');
    const al = d.alerts||[];
    if(box){
      if(!pend.length && !applied.length && !al.length){ box.innerHTML=''; }
      else{
        const chip=(t,c)=>'<span style="border:1px solid '+c+';color:'+c+';border-radius:10px;padding:1px 8px;font-size:10px">'+t+'</span>';
        let h='<div style="margin-top:8px;display:flex;align-items:center;flex-wrap:wrap;gap:6px"><span style="font-size:11px;font-weight:700;color:var(--ac)">⚡ 自动录入</span>';
        h+=chip('待命 '+pend.length, '#e8b93e');
        if(applied.length) h+=chip('已自动录 '+applied.length, '#00a884');
        if(al.length) h+=chip('⚠ 待人工 '+al.length, '#ff4070');
        if(pend.length) h+='<button class="btn-small" onclick="cjAutoClear()" title="清空所有待录推荐 (不影响已录入的仓位)">停用清空</button>';
        h+='</div>';
        const row=r=>'<div style="font-size:10.5px;color:var(--tx3);margin-left:14px;margin-top:2px">'+(r.status==='applied'?'✅':'⏳')+' '+(r.resolved_title||r.slug||'?')+' · '+(r.side||'')+' · q '+(r.q!=null?Math.round(r.q*100)+'%':'?')+' · '+(r.stop_loss_tier||'-')+(r.resolved_title?'':' · <span style="color:#e8b93e">slug未解析</span>')+(r.status==='applied'?' · <span style="color:#00a884">已自动录入</span>':'')+'</div>';
        h+=pend.map(row).join('')+applied.slice(0,5).map(row).join('');
        if(al.length) h+=al.map(a=>'<div style="font-size:10.5px;color:#ffd766;margin-left:14px;margin-top:2px">⚠ '+(a.title||a.slug||'?')+' ('+(a.side||'?')+') — '+(a.note||'')+' → 手动录: 推荐还在草稿就点「📌 录入持仓」, 不在就去持仓面板补</div>').join('');
        box.innerHTML=h;
      }
    }
    // 已自动录入的推荐: 从浏览器草稿里拿掉 (跟手动录入成功同款行为)
    if(applied.length && cjRecs.length){
      const done=new Set(applied.map(r=>(r.slug||'')+'|'+(r.side||'').toUpperCase()));
      const before=cjRecs.length;
      cjRecs=cjRecs.filter(r=>!done.has((r.slug||'')+'|'+(r.side||'').toUpperCase()));
      if(cjRecs.length!==before){ cjRenderRecs(); cjPersist(); showT('ok','⚡ 自动录入完成 '+(before-cjRecs.length)+' 条, 已从草稿移除'); }
    }
    // 失配琥珀闪窗: 内容变了才重新弹 (点过「知道了」的同一批不再弹)
    const pop=document.getElementById('autorec-pop'), txt=document.getElementById('autorec-pop-text');
    if(al.length && pop && txt){
      const msg='⚡ 自动录入: '+al.length+' 个新仓位没配上推荐, 请手动录入 — '+al.map(a=>(a.title||a.slug||(a.token||'').slice(0,10))).join(' · ');
      let dism=''; try{ dism=localStorage.getItem('pm_autorec_dismiss')||''; }catch(_){}
      if(msg!==dism){ txt.textContent=msg; pop.style.display='block'; }
    } else if(pop){ pop.style.display='none'; try{ localStorage.removeItem('pm_autorec_dismiss'); }catch(_){} }
  }).catch(()=>{});
}
window.addEventListener('load', cjAutoLoad);
setInterval(cjAutoLoad, 20000);
...
# v7.5.4: 交易复盘 HTML 报告 (手机友好, 实时从 DB 渲染). {} 是 .format 占位; CSS 里的花括号转义成 {{ }}.
_REPORT_HTML = """<!DOCTYPE html><html lang="zh"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>交易复盘 · Polymarket</title>
<style>
:root{{--bg:#0e1420;--sf:#161e2e;--sf2:#1d2740;--bd:#2a3550;--tx:#e6ebf5;--tx2:#9fb0cc;--tx3:#6b7a99;--g:#3fd08a;--r:#ff6b7d;--nv:#8fb4ff}}
*{{box-sizing:border-box}}body{{margin:0;background:var(--bg);color:var(--tx);font-family:-apple-system,"PingFang SC",system-ui,sans-serif;font-size:15px;line-height:1.5}}
.wrap{{max-width:940px;margin:0 auto;padding:16px}}
h1{{font-size:21px;margin:0 0 2px}}.sub{{color:var(--tx3);font-size:12px;margin-bottom:14px}}
h2{{font-size:16px;color:var(--nv);margin:24px 0 8px;border-left:3px solid var(--nv);padding-left:9px}}
.cards{{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}}
.card{{background:var(--sf);border:1px solid var(--bd);border-radius:12px;padding:12px 14px}}
.card .v{{font-size:24px;font-weight:800}}.card .l{{color:var(--tx2);font-size:12px;margin-top:2px}}.card .s{{color:var(--tx3);font-size:11px;margin-top:1px}}
.g{{color:var(--g)}}.r{{color:var(--r)}}.d{{color:var(--tx3)}}
.box{{background:var(--sf);border:1px solid var(--bd);border-radius:12px;padding:11px 14px;margin:9px 0;font-size:14px}}
.hl{{background:var(--sf2);border-left:3px solid var(--g)}}
.tw{{overflow-x:auto;-webkit-overflow-scrolling:touch;margin:9px 0;border:1px solid var(--bd);border-radius:10px}}
table{{border-collapse:collapse;width:100%;font-size:13px}}
th{{background:var(--sf2);color:var(--tx2);text-align:left;padding:8px 10px;font-weight:600;white-space:nowrap}}
td{{padding:7px 10px;border-top:1px solid var(--bd);white-space:nowrap}}
tbody tr:nth-child(even){{background:rgba(255,255,255,.02)}}
.tj{{font-variant-numeric:tabular-nums}}
details{{margin:9px 0}}summary{{cursor:pointer;color:var(--nv);padding:8px 0;font-weight:600}}
.note{{color:var(--tx3);font-size:12px;margin:14px 0 30px}}
</style></head><body><div class="wrap">
<h1>📊 交易复盘报告</h1>
<div class="sub">2026 年 5 月中旬 (05-15) → {today} · v{ver} · 实时从数据库渲染 · 含真实盈亏(仅你可见)</div>
<div class="cards">
 <div class="card"><div class="v">{n}</div><div class="l">已平仓笔数</div></div>
 <div class="card"><div class="v">{tot}</div><div class="l">累计盈亏</div></div>
 <div class="card"><div class="v">{prof_rate}%</div><div class="l">赚钱率 (真赚到钱)</div><div class="s">{nw} 赚 / {nl} 亏</div></div>
 <div class="card"><div class="v" style="color:var(--nv)">{dir_rate}%</div><div class="l">方向对率 (押对方向)</div><div class="s">{ncor}/{nres} 已结算押对</div></div>
</div>
<div class="box note" style="margin:8px 0 0">方向对率(押对方向)高于赚钱率(真赚到钱)= 你押对了却常没赚到 → 卖飞。</div>

<h2>① 价格大跌时 q 会不会跟着跌到坑底?(反锚定验证)</h2>
<div class="box hl"><b>结论: 基本不会跟着跌。</b> 价格平均跌 <b>{pdrop}%</b> 时 q 只降 <b>{qdrop}pp</b>(约价格跌幅 1/3);重评后 new_q 平均比坑底现价高 <b>+{gap}pp</b>,{above}/{nrvq}={above_pct}% 没锚坑底。价格跌幅 ↔ q跌幅 相关性 <b>{corr}</b>(≈0=独立判断,不是机械跟跌)。</div>
<div class="box">中枢直接对比:坑底现价 <b>{pit}%</b> · 大跌前中枢 <b>{center}%</b> · 重评后 new_q <b>{newqc}%</b> → new_q 贴着「大跌前中枢」而非坑底现价 = 反锚定有效。</div>

<h2>② 亏损归因 — {nloss} 笔亏 ${tot_loss}(只看是否赚钱)</h2>
<div class="box"><b>核心:{flew}/{lres}={flew_pct}% 的亏损, 方向其实是对的(卖飞), 不是押错。</b> 你亏钱主要栽在"出场太急", 不是"选仓"。</div>
<div class="tw"><table><thead><tr><th>出场方式</th><th>亏损笔数</th><th>合计亏</th><th>其中卖飞</th></tr></thead><tbody>{er_rows}</tbody></table></div>

<h2>③ 按标签 (下的都是什么仓 / 哪个赚)</h2>
<div class="tw"><table><thead><tr><th>标签</th><th>笔数</th><th>盈亏</th><th>方向对</th><th>赚钱</th></tr></thead><tbody>{tag_rows}</tbody></table></div>

<h2>④ 自动重评</h2>
<div class="box">{nrv} 次重评 / 覆盖 {npos} 个仓 · 决策: update_q {dec_uq} · cancel_autostop {dec_cx} · exit {dec_ex} · hold {dec_ho}</div>
<div class="tw"><table><thead><tr><th>被反复重评的仓</th><th>评次</th><th>坑底现价 → 重评q</th><th>大跌前中枢</th></tr></thead><tbody>{traj_rows}</tbody></table></div>

<h2>⑤ 每一笔明细 ({n} 笔)</h2>
<details><summary>▸ 点开明细表 (方向对=绿 / 错=红 / 未结算=灰)</summary>
<div class="tw"><table><thead><tr><th>#</th><th>名称</th><th>标签</th><th>方向</th><th>入场→出场</th><th>盈亏$</th><th>方向判定</th></tr></thead><tbody>{det_rows}</tbody></table></div>
</details>
<div class="note">实时来自 closed_positions + auto_reeval_suggestions。刷新即最新。</div>
</div></body></html>"""
...
def create_app():
    ...
    def auto_reeval_confirm():
        ...
        from modules.auto_reeval import guard_event_driven_exit, guard_event_driven_cancel
        ...
        # v7.5.3: event_driven 禁止 API cancel_autostop → 降级 (保 -60% 兜底; 用户手动关不受限)
        action, _dc, _wc = guard_event_driven_cancel(action, s, _tier)
        if _dc:
            log.warning(f"auto_reeval_confirm id={sug_id}: {_wc}")
        try:
            if action == "exit":
                exe = Executor.get()
                pos = next((p for p in exe.get_positions() if p.get("asset") == token_id), None)
    ...
    # v7.5.4: 交易复盘 HTML 网页 — 手机/其他设备直接看 (走 Tailscale + 密码鉴权; 实时从 DB 渲染, 不依赖任何文件)
    @app.route("/report")
    def trade_report():
        import sqlite3 as _sq, statistics as _st, re as _re
        from collections import defaultdict as _dd, Counter as _ct
        from modules.db import get_conn
        CUT = "2026-05-15"; TODAY = "2026-07-13"
        conn = get_conn(); conn.row_factory = _sq.Row
        rows = [dict(r) for r in conn.execute("SELECT * FROM closed_positions WHERE exit_at>=? ORDER BY exit_at DESC", (CUT,))]
        RV = [dict(r) for r in conn.execute("SELECT * FROM auto_reeval_suggestions WHERE created_at>=? ORDER BY created_at", (CUT,))]
        conn.close()
        def nn(s):
            if not s: return "(无名)"
            s = _re.sub(r"-\d{6,}$", "", s); s = _re.sub(r"^will-", "", s); return s.replace("-", " ")
        def esc(s): return (str(s) if s is not None else "").replace("<", "&lt;").replace(">", "&gt;")
        def m(v):
            v = v or 0; return f'<b class="{"g" if v>=0 else "r"}">{"+" if v>=0 else "-"}${abs(v):.2f}</b>'
        n = len(rows); total = sum(r["realized_pnl_usd"] or 0 for r in rows)
        W = [r for r in rows if (r["realized_pnl_usd"] or 0) > 0]; L = [r for r in rows if (r["realized_pnl_usd"] or 0) < 0]
        resolved = [r for r in rows if r["is_resolved"]]; correct = [r for r in resolved if r["is_correct"]]
        prof_rate = len(W)/n*100 if n else 0; dir_rate = len(correct)/len(resolved)*100 if resolved else 0
        tot_loss = sum(r["realized_pnl_usd"] for r in L)
        Lres = [r for r in L if r["is_resolved"]]; Lflew = [r for r in Lres if r["is_correct"]]
        ...
        qdrop = _st.mean([r["orig_q"]-r["new_q"] for r in RVo])*100 if RVo else 0
        pdrop = _st.mean([abs(r["loss_pct"] or 0) for r in RVo])*100 if RVo else 0
        ...
        center = _st.mean([r["pre_dump_center"] for r in RVc])*100 if RVc else 0
        pit = _st.mean([r["cur_price"] for r in RVc])*100 if RVc else 0
        newqc = _st.mean([r["new_q"] for r in RVc])*100 if RVc else 0
        rv_pos = _dd(list)
        for r in RV: rv_pos[r["token_id"]].append(r)
        rv_dec = _ct(r["action"] for r in RV if r["action"])
        # ---- 表格拼装 ----
        def tbl(head, body):
            h = "".join(f"<th>{x}</th>" for x in head)
            return f'<div class="tw"><table><thead><tr>{h}</tr></thead><tbody>{body}</tbody></table></div>'
        # 标签
        tags = _dd(lambda: {"n":0,"res":0,"cor":0,"prof":0,"pnl":0.0})
        for r in rows:
            t = tags[r["tag"] or "(无标签)"]; t["n"]+=1; t["pnl"]+=r["realized_pnl_usd"] or 0
            t["res"]+=1 if r["is_resolved"] else 0; t["cor"]+=1 if (r["is_resolved"] and r["is_correct"]) else 0
            t["prof"]+=1 if (r["realized_pnl_usd"] or 0)>0 else 0
        tag_rows = "".join(f'<tr><td>{esc(k)}</td><td>{v["n"]}</td><td>{m(v["pnl"])}</td><td>{v["cor"]}/{v["res"]}</td><td>{v["prof"]}/{v["n"]}</td></tr>'
                           for k,v in sorted(tags.items(), key=lambda x:-x[1]["pnl"]))
        # 出场方式(亏损)
        ner = {"FORCE_EXIT:reeval_exit":"重评清仓","AUTO_REEVAL:exit":"自动重评清仓","AUTO_REEVAL:exit(离线自动)":"自动重评清仓(离线)","REBUILT_FROM_TRADES":"早期回填(非bot)","FORCE_EXIT:manual_liquidate":"手动清仓","STOP_LOSS":"止损","TIME_STOP":"时间止损","TAKE_PROFIT_HALF":"止盈卖半"}
        er = _dd(lambda: {"n":0,"pnl":0.0,"flew":0})
        for r in L:
            e = er[r["exit_reason"] or "?"]; e["n"]+=1; e["pnl"]+=r["realized_pnl_usd"]
            if r["is_resolved"] and r["is_correct"]: e["flew"]+=1
        er_rows = "".join(f'<tr><td>{esc(ner.get(k,k))}</td><td>{v["n"]}</td><td>{m(v["pnl"])}</td><td>{v["flew"]}</td></tr>'
                          for k,v in sorted(er.items(), key=lambda x:x[1]["pnl"]))
        # 多次重评轨迹
        traj_rows = ""
        for tid,lst in sorted(rv_pos.items(), key=lambda x:-len(x[1])):
            if len(lst) < 2: continue
            tj = " → ".join(f'{r["cur_price"]:.2f}→<b>{int(r["new_q"]*100)}%</b>' for r in lst if r["new_q"] is not None)
            ct = " / ".join(f'{r["pre_dump_center"]:.2f}' for r in lst if r.get("pre_dump_center")) or "—"
            traj_rows += f'<tr><td>{esc(nn(lst[0]["slug"])[:40])}</td><td>{len(lst)}</td><td class="tj">{tj}</td><td>{ct}</td></tr>'
        # 每笔明细
        det_rows = ""
        for i,r in enumerate(rows,1):
            j = "方向对" if (r["is_resolved"] and r["is_correct"]) else "方向错" if r["is_resolved"] else "未结算"
            jc = "g" if (r["is_resolved"] and r["is_correct"]) else "r" if r["is_resolved"] else "d"
            det_rows += (f'<tr><td>{i}</td><td>{esc(nn(r["market_slug"])[:56])}</td><td>{esc(r["tag"] or "—")}</td>'
                         f'<td>{esc((r["side"] or "?").upper())}</td><td>${(r["avg_entry_price"] or 0):.2f}→${(r["exit_price"] or 0):.2f}</td>'
                         f'<td>{m(r["realized_pnl_usd"])}</td><td class="{jc}">{j}</td></tr>')
        html = _REPORT_HTML.format(
            ver=VERSION, today=TODAY, n=n, tot=m(total), prof_rate=f"{prof_rate:.0f}", nw=len(W), nl=len(L),
            dir_rate=f"{dir_rate:.0f}", ncor=len(correct), nres=len(resolved),
            nloss=len(L), tot_loss=f"{abs(tot_loss):.2f}", flew=len(Lflew), lres=len(Lres),
            flew_pct=f"{len(Lflew)/len(Lres)*100:.0f}" if Lres else "0",
            pdrop=f"{pdrop:.0f}", qdrop=f"{qdrop:.0f}", gap=f"{gap_avg:.0f}", above=above, nrvq=len(RVq),
            above_pct=f"{above/len(RVq)*100:.0f}" if RVq else "0", corr=f"{corr:+.2f}",
            pit=f"{pit:.0f}", center=f"{center:.0f}", newqc=f"{newqc:.0f}", nrv=len(RV), npos=len(rv_pos),
            dec_uq=rv_dec.get("update_q",0), dec_cx=rv_dec.get("cancel_autostop",0), dec_ex=rv_dec.get("exit",0), dec_ho=rv_dec.get("hold",0),
            tag_rows=tag_rows, er_rows=er_rows, traj_rows=traj_rows, det_rows=det_rows)
        return html
    ...
    def record_position():
        ...
        try:
            ...
            # v7.4.6: 入场价【一定】用 Polymarket 当前实时加权均价 (= 这次买的), 不信前端传的 data-avg。
            #   原因: 页面没刷新 / 二次入场 (之前持有过同一 token 又买回) 时 data-avg 可能还是上一轮旧价,
            #   会把"以前的入场价"写进新仓 (用户 2026-07-08 踩到 US-Iran: 旧 $0.71 vs 这次 $0.52)。
            #   实时拉到就覆盖前端值; 拉不到 (罕见) 才退回前端值; 都没有才报错。
            token_id = data.get("token_id", "")
            try:
                exe = Executor.get()
                for p in exe.get_positions():
                    if p.get("asset") == token_id:
                        ...
                        if live_avg > 0:
                            if entry_price > 0 and abs(live_avg - entry_price) > 0.001:
                                log.warning(f"record_position: 用 Polymarket 实时均价 ${live_avg:.4f} 覆盖前端传的 ${entry_price:.4f} (token {token_id[:16]}; 防旧价/二次入场污染)")
                            entry_price = live_avg
                        break
            except Exception as e:
                log.warning(f"record_position live avg fetch failed (退回前端 entry_price): {e}")
            if entry_price <= 0:
                return jsonify({"ok": False, "message": "❌ 无法从 Polymarket 拉到当前均价, 前端也没传有效入场价, 请刷新页面重试"})
    ...
    # v7.5.1: 把 Claude 给的 slug 解析成真实市场 (Claude 常给烂/旧 slug — 实测给过
    # "...byptptpt-<旧时间戳>" 这种不存在的)。精确查 → 查不到就 public-search 模糊搜
    # (归一化去掉尾部时间戳后缀, 按公共前缀打分, end_date 决胜同名兄弟市场)。
    # 返回 {resolved_slug, resolved_title, token_yes, token_no} 或 None (查不到, 存入后靠兜底匹配)。
    def _resolve_rec_market(slug, end_date=None):
        import requests as _req, re as _re, json as _json2
        def _norm(s):
            return _re.sub(r"-\d{8,}$", "", (s or "").strip().lower())
        def _pack(m):
            try:
                toks = _json2.loads(m.get("clobTokenIds") or "[]")
                outs = [str(o).upper() for o in _json2.loads(m.get("outcomes") or '["Yes","No"]')]
            except Exception:
                return None
            if len(toks) < 2:
                return None
            iy = outs.index("YES") if "YES" in outs else 0
            ino = outs.index("NO") if "NO" in outs else 1
            return {"resolved_slug": m.get("slug") or "", "resolved_title": m.get("question") or "",
                    "token_yes": toks[iy], "token_no": toks[ino]}
        try:
            r = _req.get("https://gamma-api.polymarket.com/markets",
                         params={"slug": slug, "limit": 1}, timeout=8).json()
            if isinstance(r, list) and r and not r[0].get("closed"):
                p = _pack(r[0])
                if p:
                    return p
        except Exception:
            pass
        try:
            q = _norm(slug).replace("-", " ")
            s = _req.get("https://gamma-api.polymarket.com/public-search",
                         params={"q": q, "limit_per_type": 10}, timeout=10).json()
            a = _norm(slug)
            cands = []
                ...
                for m in (ev.get("markets") or []):
                    ...
                    for x, y in zip(a, b):
                        if x != y:
                            break
                        cp += 1
                    if cp >= 15 and cp >= 0.6 * min(len(a), len(b)):
                        cands.append((cp, m))
            if not cands:
                return None
            cands.sort(key=lambda t: -t[0])
            top = [c for c in cands if c[0] >= cands[0][0] - 2]
            if len(top) > 1 and end_date:  # 兄弟市场 (by-july-15/31/august-31) 用结算日决胜
                same_day = [c for c in top if (c[1].get("endDate") or "")[:10] == str(end_date)[:10]]
                if same_day:
                    top = same_day
            return _pack(top[0][1])
        except Exception:
            return None

    # v7.5 ⚡自动录入: 推荐存服务器 (24h), monitor 心跳自动配新仓位写 meta; 见 monitor._auto_record_from_pending
    @app.route("/api/pending_recs", methods=["GET", "POST"])
    def pending_recs_api():
        from modules.db import add_pending_recs, get_pending_recs, expire_pending_recs, get_app_state
        import json as _json
        if flask_request.method == "POST":
            data = flask_request.get_json() or {}
            recs = data.get("recs") or []
            if not isinstance(recs, list) or not recs:
                return jsonify({"ok": False, "message": "没有可存的推荐"})
            resolved, unresolved = [], []
            for r in recs:
                if not isinstance(r, dict) or not r.get("slug"):
                    continue
                info = _resolve_rec_market(r.get("slug"), r.get("end_date"))
                if info:
                    r.update(info)
                    resolved.append({"slug": r.get("slug"), "title": info["resolved_title"]})
                    if info["resolved_slug"] != r.get("slug"):
                        log.info(f"pending_rec slug 解析修正: {r.get('slug')} → {info['resolved_slug']}")
                else:
                    unresolved.append(r.get("slug"))
            saved = add_pending_recs(recs)
            if not saved:
                return jsonify({"ok": False, "message": "0 条通过校验 (每条需要 slug + side(YES/NO) + q(0-1))"})
            return jsonify({"ok": True, "saved": saved, "resolved": resolved, "unresolved": unresolved})
        expire_pending_recs()
        rows = get_pending_recs(None)
        try:
            alerts = _json.loads(get_app_state("auto_record_alerts") or "[]")
        except Exception:
            alerts = []
        return jsonify({"ok": True, "recs": rows, "alerts": alerts})

    @app.route("/api/pending_recs/clear", methods=["POST"])
    def pending_recs_clear_api():
        from modules.db import clear_pending_recs, set_app_state
        n = clear_pending_recs()
        set_app_state("auto_record_alerts", "[]")
