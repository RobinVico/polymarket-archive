...
## Current version at a glance (v7.5.4)
...
Auto-sell rules (priority high → low):
  ...
  2.  STOP_LOSS           tiered by LLM classification at entry:
        ...
        hybrid       (mixed)     → TRAILING: ≥35% off peak + 6-tick confirm → sell DIRECTLY on breach (v7.4.5, no reeval)
        ...
        (auto-reeval ON, ONLY event_driven & convergent: a breached stop defers to PENDING_REEVAL → the reeval decides;
         ...
         hybrid & unclassified: a breached stop sells DIRECTLY, no reeval — v7.4.5)
...
**Pages**: `/` desktop dashboard (scan / sizing / positions / events / logs) · `/panel` secondary-screen control panel · `/m` read-only mobile page (mobile UAs auto-redirect) · `/history` closed-position analytics (resolution tracking, profit rate vs direction accuracy, calibration, Chart.js trends) · `/paper` paper-trading book · `/tags` dynamic hot-tag board · `/report` trade-review report (live-rendered from the DB, mobile-friendly).
...
**New in v7.2–v7.5**: positions card split into 3 tabs — read-only holdings / an ops panel (all edits + one-click apply of a pasted Claude reeval JSON) / reeval mode (7.2); `/history` stats overhaul — Chart.js monthly trend + cumulative line, PnL distribution, sold-too-early analysis, by-exit-category table, `closed_positions` rebuilt 1:1 from Polymarket fills (7.3); paper-trading lifecycle — active vs history with peak price, prediction-correct verdicts and simulated win-rate (7.4.0); real-money exit tightening — event-driven "double vs 0.92-half, whichever first" (7.4.2), post-half-sell 0.782 protection (7.4.1), a loose −60% event stop replacing floor-only (7.4.3), hybrid switched to a 35% trailing stop off the peak (7.4.4), and its stop-breach now sells directly without reeval — only event_driven + convergent still route a breach to reeval (7.4.5); auto-reeval is dual-model — Claude authoritative + Zhipu GLM on the `/api_reeval` comparison page; ⚡ **auto-record** — save the day's JSON recs server-side (24h) and newly appearing positions get their full metadata (q/confidence/tier/cluster/tag) filled automatically by the heartbeat, unmatched ones flash an amber alert (7.5.0); recorded entry price is always re-pulled live from Polymarket (7.4.6). Version numbering now has a single source (`modules/version.py`). Full changelog: [TECHNICAL_REPORT.md](TECHNICAL_REPORT.md) header · [技术报告.md](技术报告.md) 版本号规则.
...
- An older v3 codebase lives at `~/polymarket-semi-auto`, frozen, not part of this repo.
