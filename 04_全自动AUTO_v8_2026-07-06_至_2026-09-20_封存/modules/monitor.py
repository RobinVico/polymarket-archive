...
#   event_driven— 事件驱动型 (政治/外交/谈判):           -50% 强行止损【直接平仓】(AUTO 2026-07-18) + $0.05 地板兜底
# meta.stop_loss_tier 为 None (老仓/未分类) → v7.4.3 默认当 hybrid (不再走 -25% legacy; 前端也禁止选空白档).
STOP_LOSS_PCT_BY_TIER = {
    "convergent": 0.20,
    "hybrid": 0.35,
    "event_driven": 0.60,  # 老"很松-60%→交重评"已被 AUTO EVENT_DRIVEN_HARD_STOP_PCT(-50% 直接平仓)取代; -30% 重评触发仍在
}
...
# AUTO 分叉 (2026-07-18 用户最高指令): 事件型跌破成本 -50% → 强行直接平仓, 早于重评/护栏/手动关止损, 任何东西越不过。
EVENT_DRIVEN_HARD_STOP_PCT = 0.50
...
class PositionMonitor:
    ...
    def _evaluate_position(self, pos, meta, breach_store=None):
        ...
        if avg and avg > 0 and "stop_loss_sold" not in executed:
            ...
            # (a2) AUTO 分叉 (2026-07-18 用户最高指令): 事件型 -50% 强行止损 —— 直接平仓, 不走重评/护栏,
            #      早于下面所有止损线判定 (含"手动关止损"), 任何东西都越不过。
            #      依据: 仓位回撤分析显示事件型跌破约 -50% 就是"回不来"的不归点。
            if tier == "event_driven" and (avg - cur_price) / avg >= EVENT_DRIVEN_HARD_STOP_PCT:
                return ("STOP_LOSS", {
                    "action": "STOP_LOSS",
                    "reason": f"事件驱动型 -{int(EVENT_DRIVEN_HARD_STOP_PCT*100)}% 强行止损: 成本${avg:.3f}→现${cur_price:.3f} (亏{(avg-cur_price)/avg*100:.0f}%),直接平仓不走重评/护栏",
                    "sell_size": size,
                    "executed_action": "stop_loss_sold",
                })
            # (b) 止损线触发判定 — convergent + hybrid 从最高价回撤+确认 (移动止损); event_driven 见上 (a2) 强行止损
            ...
            if not _autostop_off:
                if tier in ("convergent", "hybrid"):
                    ...
                    _breached = True                                  # legacy/未分类 入场锚 (event_driven 已由上面 -50% 强行止损接管)
            if _breached:
                # v7.4.5: 混合型 = 砸穿止损线【直接平仓】。AUTO 2026-07-18 用户拍板: 收敛型也改成砸穿直接平仓
                #         (从最高价回撤 20%/≤3天12% + 确认 → 直接卖, 不再交重评)。事件型 -50% 强行止损见上面 (a2)。
                _direct_stop = tier in ("convergent", "hybrid")
                try:
                ...
                if tier in ("convergent", "hybrid"):
                    ...
                    _why = "到值直接平仓" if _direct_stop else "重评未启用"
                    return ("STOP_LOSS", {
                        "action": "STOP_LOSS",
    ...
    def _maybe_trigger_auto_reeval(self, pos, meta, state):
        ...
        # AUTO 2026-07-18 用户拍板: 盘中"亏损触发重评"只留给【事件型】。
        #   收敛型/混合型的止损 = 砸穿直接平仓 (见 _evaluate_position), 不再盘中重评;
        #   每天 15:00 全仓巡检重评对【所有】仓照旧 (auto_daily_reeval, 是另一条路, 不受此影响)。
        tier = (meta or {}).get("stop_loss_tier") or "hybrid"   # 未分类默认当 hybrid
        ...
        from modules.db import (has_inflight_auto_reeval, save_auto_reeval_pending,
                                get_reeval_watch_loss, set_reeval_watch_loss)
        # 进行中(调研中/待确认/手动卡) → 锁住, 不重复起 API
        if has_inflight_auto_reeval(token_id):
            return
        # AUTO 2026-07-18: 事件型【任何盈亏水平】, 从"持有期最好点(最低 loss_pct)"回撤 ≥ RETRIGGER_DROP_PCT(默认5pp)
        #   → 触发 1 次重评。基线 = 上次触发后见过的最低 loss_pct; 创新低(涨回去)就下移基线;
        #   loss ≥ 基线+5pp → 触发, 并把基线重置到当前 (→ 单次暴跌只算 1 次, 不按 5pp 整除多次触发)。
        #   无时间冷却 (纯按跌幅); 40%→(涨回)35%→(再跌)40% 会再触发 (从 35 的最好点又跌 5pp)。
        _watch = get_reeval_watch_loss(token_id)
        if _watch is None or loss_pct < _watch:
            set_reeval_watch_loss(token_id, loss_pct)   # 首次见到 / 创新低(更好) → 记为新基线, 本拍不触发
            return
        if loss_pct < _watch + auto_reeval.RETRIGGER_DROP_PCT:
            return                                       # 从最好点还没回撤够 5pp → 不触发
        set_reeval_watch_loss(token_id, loss_pct)       # 达标 → 触发 (基线重置到当前, 防单次暴跌多次触发)
    ...
    def run_loop(self):
        ...
        log.info(f"      convergent   (真相收敛型) → 移动止损: 从最高价回撤 ≥{int(TRAILING_STOP_PCT_CONVERGENT*100)}% (≤3天 {int(TRAILING_STOP_PCT_CONVERGENT_NEAR*100)}%) + 连{TRAILING_CONFIRM_ROUNDS}拍确认 → 【直接平仓】(AUTO 2026-07-18, 不再交重评)")
        log.info(f"      hybrid       (混合型)     → 入场锚 -{int(STOP_LOSS_PCT_BY_TIER['hybrid']*100)}%")
        log.info(f"      event_driven (事件驱动型) → -{int(EVENT_DRIVEN_HARD_STOP_PCT*100)}% 强行止损【直接平仓·任何东西越不过】(AUTO 2026-07-18) + 价 < ${EVENT_DRIVEN_FLOOR_PRICE:.2f} 地板兜底")
        log.info(f"      (未分类老仓位 fallback 入场锚 -{int(STOP_LOSS_PCT_LEGACY*100)}%)")
        log.info(f"  2b. AUTO 2026-07-18: 止损砸穿=直接平仓 (收敛/混合回撤确认 + 事件-50%, 不再进 PENDING_REEVAL); 事件型另: 盘中从'最好点'回撤≥5pp 触发1次重评(重评说卖立刻卖), 每天15:00全仓巡检照旧")
