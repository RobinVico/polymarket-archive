...
GAP_BREAK_SEC = int(os.environ.get("AUTO_CURVE_GAP_BREAK_SEC", str(6 * 3600)))
...
def _adjusted_portfolio_history(rng):
    ...
    # 2026-08-27 用户: 「把我不在的这几天从曲线上删掉」——
    #   ① 剔除开机瞬间的假点: 持仓值=0 但前后两点都有仓 (那一拍只读到现金, 持仓还没拉回来,
    #      会在曲线上戳一个假崩盘坑; 08-27 重启时 bench 就被戳了一个 $28.85 的假坑)。
    #   ② 停机空档不再拉直线跨过去: 相邻两点隔了 > GAP_BREAK_SEC = bot 那段时间关着、无人打点,
    #      中间插一个 y=null 的断点, Chart.js (spanGaps 默认 false) 就会把线断开。
    #      ⚠️ 只动服务端数据; 前端 loadChart 是 8.5.1 用户钦定的共用部件, 一个字不改。
    clean = []
    for i, r in enumerate(rows):
        tv = r.get("total_value")
        if (not tv and 0 < i < len(rows) - 1
                and (rows[i - 1].get("total_value") or 0) > 1 and (rows[i + 1].get("total_value") or 0) > 1):
            continue
        clean.append(r)
    rows = clean
    ...
    # 停机空档: 插 y=null 断点, 让曲线断开而不是拉一条假直线 (见上方 ②)
    broken, gaps = [], []
    for i, p in enumerate(out):
        if i and p["ts"] - out[i - 1]["ts"] > GAP_BREAK_SEC:
            gaps.append((out[i - 1]["ts"], p["ts"]))
            broken.append({"ts": (out[i - 1]["ts"] + p["ts"]) // 2, "assets_total": None,
                           "cost_line": None, "cash": None, "total_value": None, "adj": 0})
        broken.append(p)
    out = broken
    note = ""
    if gaps:
        note = "已断开 %d 段停机空档" % len(gaps)
    if deposits:
        note = (note + " · " if note else "") + "已校正出入金: " + ", ".join(
