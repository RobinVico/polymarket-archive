...
def save_position_meta(token_id, market_slug, side, entry_price, tp, end_date, initial_size, notes="", entry_reason="", claude_raw_estimate=None):
    ...
    conn.execute("""INSERT OR REPLACE INTO position_meta 
        (token_id, market_slug, side, entry_price, tp, target_gap, end_date, initial_size, created_at, notes, entry_reason, claude_raw_estimate, reeval_status) 
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?, 'pending')""",
        (token_id, market_slug, side, entry_price, tp, target_gap, end_date, initial_size, datetime.now().isoformat(), notes, entry_reason, claude_raw_estimate))
    conn.commit(); conn.close()
