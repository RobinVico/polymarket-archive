...
def _norm_slug(s):
    """v7.5.1: slug 归一化 — 去掉 Polymarket 自动生成的尾部时间戳后缀 + 小写。自动录入模糊匹配用。"""
    import re as _re
...
class PositionMonitor:
    ...
    def _evaluate_position(self, pos, meta, breach_store=None):
        ...
        if avg and avg > 0 and "stop_loss_sold" not in executed:
            ...
            if _breached:
                # v7.4.5: 混合型 = 砸穿止损线【直接平仓】(不走 API 重评, 用户 2026-07-08 定, 数据支持混合型扛输最差);
                #         收敛型/事件型 = 仍先交重评 (它们扛住多半会反弹, 尤其收敛型有 -64%→+92% 的救援案例)。
                _direct_stop = (tier == "hybrid")
                try:
                    ...
                    _reeval_on = False
                if _reeval_on and not _direct_stop:
                    # 收敛型/事件型: 不盲卖 → 进 pending, 交给重评决定 (大跳也先进这个状态; _maybe_trigger 见 PENDING_REEVAL 会放行)
                    return ("PENDING_REEVAL", None)
                # 混合型到值直接卖 / 或重评未启用 → 硬止损
                if tier in ("convergent", "hybrid"):
                    peak = meta.get("peak_price") or avg or cur_price
                    dd = (peak - cur_price) / peak * 100 if peak else 0
                    _why = "混合型到值直接平仓" if _direct_stop else "重评未启用"
                    return ("STOP_LOSS", {
                        "action": "STOP_LOSS",
                        "reason": f"[{tier} 移动止损] 从最高${peak:.3f}回撤{dd:.0f}% (cur${cur_price:.3f}),自动全卖 ({_why})",
                        "sell_size": size,
                        "executed_action": "stop_loss_sold",
                    })
    ...
    def _auto_record_from_pending(self, positions):
        """v7.5 ⚡自动录入: 主页「⚡ 存入自动录入」存下的推荐 (pending_recs, 24h 有效) ↔ 新出现的
        无 meta 仓位, 按 slug+side 自动配对:
          配上 → 写全套 position_meta (q/信心/止损档/cluster/tag, 与 /api/record_position 同口径)
          配不上 (自动录入待命中出现的陌生新仓) → 写 app_state.auto_record_alerts, 前端琥珀闪窗提醒
          提醒是粘性的: 一旦挂上, 直到该仓被补录 meta 或仓位消失才撤 (哪怕待录清空了也留着)
        🔒 只写元数据, 永不下单/卖出; 绝不覆盖已有 meta; q ≤ 成本价的不自动填 (TP 方向错), 转人工。"""
        from modules.db import (get_pending_recs, expire_pending_recs, mark_pending_rec,
                                save_position_meta, set_app_state, get_app_state)
        import json as _json
        expire_pending_recs()
        recs = get_pending_recs("pending")
        try:
            alerts_prev = {a.get("token"): a for a in _json.loads(get_app_state("auto_record_alerts") or "[]")
                           if a.get("token")}
        except Exception:
            alerts_prev = {}
        pos_by_tok = {p.get("asset"): p for p in positions if p.get("asset")}
        # 粘性保留: 老提醒只要仓位还在且还没 meta 就不撤
        alerts_new = {tok: a for tok, a in alerts_prev.items()
                      if tok in pos_by_tok and not get_position_meta(tok)}
        for pos in positions:
            tok = pos.get("asset") or ""
            if not tok or get_position_meta(tok):
                continue
            slug = (pos.get("slug") or "").strip()
            side = (pos.get("side") or "").strip().upper()
            now_iso = datetime.now(timezone.utc).isoformat()
            # --- v7.5.1 三层匹配阶梯 (Claude 常给烂/旧 slug, 实测 "byptptpt-旧时间戳" 直接配空):
            #   ① token 精确 (存入时已解析到真实市场)  ② slug / resolved_slug 精确
            #   ③ 归一化前缀模糊 (对实际仓位 slug 打分, 唯一赢家才敢自动填, 平手转人工) ---
            hit = None; how = ""; side_conflict = None
            for r in recs:
                if r.get("token_yes") and tok in (r.get("token_yes"), r.get("token_no")):
                    if (r["side"] or "").upper() == side:
                        hit, how = r, "token"
                    else:
                        side_conflict = r   # 市场对了但方向不对 → 绝不自动填, 转人工
                    break
            if hit is None and side_conflict is None:
                hit = next((r for r in recs
                            if (r["slug"] == slug or (r.get("resolved_slug") or "") == slug)
                            and (r["side"] or "").upper() == side), None)
                if hit:
                    how = "slug"
            if hit is None and side_conflict is None and slug:
                a = _norm_slug(slug)
                scored = []
                for r in recs:
                    if (r["side"] or "").upper() != side:
                        continue
                    best = 0
                    for cand in (r["slug"], r.get("resolved_slug") or ""):
                        b = _norm_slug(cand)
                        if not b:
                            continue
                        cp = 0
                ...
                if scored:
                    ...
                    if len(scored) == 1 or scored[0][0] - scored[1][0] > 2:
                        hit, how = scored[0][1], "fuzzy"
                    else:
                        if tok not in alerts_new:
                            alerts_new[tok] = {"token": tok, "slug": slug, "side": side,
                                               "title": (pos.get("title") or "")[:60],
                                               "note": "多个待录推荐都相似, 分不出唯一, 需人工挑", "ts": now_iso}
                        continue
            if side_conflict is not None:
                if tok not in alerts_new:
                    alerts_new[tok] = {"token": tok, "slug": slug, "side": side,
                                       "title": (pos.get("title") or "")[:60],
                                       "note": f"对上市场但方向不一致: 推荐 {side_conflict['side']} 实际买了 {side}",
                                       "ts": now_iso}
                continue
            if hit is None:
                if recs and tok not in alerts_new:   # 待命中才产生新提醒; 平时 NO_META 的安静提示维持原样
                    alerts_new[tok] = {"token": tok, "slug": slug, "side": side,
                                       "title": (pos.get("title") or "")[:60],
                                       "note": "没对上任何待录推荐", "ts": now_iso}
                continue
            avg = float(pos.get("avg_price") or 0)
            q = float(hit["q"] or 0)
            if not (0 < avg < 1) or q <= avg:
                if tok not in alerts_new:
                    alerts_new[tok] = {"token": tok, "slug": slug, "side": side,
                                       "title": (pos.get("title") or "")[:60],
                                       "note": f"对上推荐但 q({q:.2f})≤成本({avg:.2f}), 需人工确认", "ts": now_iso}
                continue
            try:
                save_position_meta(
                    token_id=tok, market_slug=slug, side=side,
                    entry_price=avg, tp=q,
                    end_date=(hit.get("end_date") or pos.get("end_date") or ""),
                    initial_size=float(pos.get("size") or 0),
                    notes="",
                    entry_reason=(f"[auto-record] {hit.get('reason') or ''}").strip()[:2000],
                    claude_raw_estimate=q,
                    original_confidence=hit.get("confidence") or None,
                    stop_loss_tier=hit.get("stop_loss_tier") or None,
                    cluster_id=hit.get("cluster_id") or None,
                    tag=hit.get("tag") or None,
                )
                mark_pending_rec(hit["id"], "applied", tok)
                recs = [r for r in recs if r["id"] != hit["id"]]
                log_event("AUTO_RECORD", slug,
                          f"⚡ 自动录入 {side} q={q:.2f} tier={hit.get('stop_loss_tier') or '-'} "
                          f"conf={hit.get('confidence') or '-'} cluster={hit.get('cluster_id') or '-'} "
                          f"(rec#{hit['id']} via {how})")
                log.info(f"⚡ auto-record: {(pos.get('title') or '')[:40]} {side} ← rec#{hit['id']} q={q:.2f} via {how}")
            except Exception as e:
                log.warning(f"auto-record save failed {slug}: {e}")
        if _json.dumps(sorted(alerts_new.keys())) != _json.dumps(sorted(alerts_prev.keys())):
            set_app_state("auto_record_alerts", _json.dumps(list(alerts_new.values()), ensure_ascii=False))
        # 用户已手动录过的推荐 → 标 superseded, 别一直待命占着
        try:
            by_ss = {((p.get("slug") or "").strip(), (p.get("side") or "").upper()): p.get("asset")
                     for p in positions}
            all_toks = {p.get("asset") for p in positions if p.get("asset")}
            for r in recs:
                sideu = (r["side"] or "").upper()
                tok2 = (by_ss.get((r["slug"], sideu))
                        or by_ss.get(((r.get("resolved_slug") or ""), sideu)))
                if not tok2 and r.get("token_yes") in all_toks:
                    tok2 = r.get("token_yes")
                if not tok2 and r.get("token_no") in all_toks:
                    tok2 = r.get("token_no")
                if tok2 and get_position_meta(tok2):
                    mark_pending_rec(r["id"], "superseded", tok2)
                    log.info(f"auto-record: rec#{r['id']} {r['slug']} 已被手动录入, 标 superseded")
        except Exception as e:
            log.warning(f"auto-record supersede check failed: {e}")
    ...
    def check_once(self):
        ...
        # === v7.5 ⚡自动录入: 待录推荐 ↔ 新仓位 自动配对 (只写元数据, 不碰钱) ===
        if positions:
            try:
                self._auto_record_from_pending(positions)
            except Exception as e:
                log.warning(f"auto-record pass failed (主决策不受影响): {e}")
    ...
    def _maybe_trigger_auto_reeval(self, pos, meta, state):
        ...
        if tier == "hybrid":
            return   # v7.4.5: 混合型(含未分类=默认hybrid) 改"到值直接平仓, 不走 API 重评" → 不给它起自动重评
