...
- **📌 文档/PPT 同步铁律 (2026-07-08 用户强调"写死": 改任何策略/代码/版本后, 下面全部同步, 一个都不能漏, 当次做完别攒着)**:
  1. `modules/version.py` VERSION (单一来源, 决定所有页面 + 文件名版本号)。
  2. `main.py` 启动日志那行策略描述 (别让它撒谎)。
  3. `modules/dashboard.py` 相关 UI (持仓 tier 下拉 title / 规则展示卡等)。
  4. `CLAUDE.md` 变更日志 + 相关设计段。
  5. `技术报告.md` **和** `TECHNICAL_REPORT.md` (中英都改, 见 [[docs-i18n-parity]])。
  6. `README.zh.md` **和** `README.md` (「当前版本概要」版本号 + 策略 ASCII 块 + 版本摘要行; 标题用 major.minor 如 v7.4, 概要块标到 patch)。
  7. `SECURITY.zh.md` **和** `SECURITY.md` ("当前 vX.Y", 仅版本/安全相关时)。
  8. **内部 PPT**: `scripts/gen_strategy_ppt.py` (VERSION + 内容) → `/tmp/pptenv/bin/python3 scripts/gen_strategy_ppt.py` 重生成 → 删旧版 pptx。
  9. **分享版 PPT**: `scripts/gen_strategy_ppt_share.py` (同一套策略内容, 页码不同 P6/P9/P10 等) → 同法重生成 → 删旧。⚠️ **最易漏: 分享版策略内容跟内部版一模一样但是独立文件, 每次改策略都要一起改+重生成**。
  10. 相关 memory (auto-reeval / 出场策略 / [[strategy-ppt-generator]] 等)。
  - **改完必验证**: 抽 PPT 正文核对新内容进没进 (含表格 `shape.table` 单元格, 别只读 text_frame) + `grep` 确认旧值无残留。(原 2026-07-07 版只覆盖 "minor + 6 文档"; 现扩成 "任何改动 + 含两个 PPT"。)
- **变更日志**: `7.1.4` (2026-07-05, patch) = 单一版本源 `version.py` + 事件实时榜加 6h/1d 档 + 自动重评建议 >48h 自动清空归档 + 修 monitor 缺失 `from modules.executor import Executor`(HEAD 已缺, 会致重启崩溃)。
  ...
  - `7.4.5` (2026-07-08, patch, 真钱 = **混合型止损不走重评, 直接平仓**): 用户看完"离线全仓都走重评"的数据研究(结合网上+历史)后决定 —— 只有**事件型 + 收敛型**砸穿止损线才进 PENDING_REEVAL 交 API 重评; **混合型(含未分类=默认hybrid)砸穿 → 直接平仓, 不走重评**。依据: 20次重评19次说hold, 但收敛型扛住2/3反弹(含 -64%→+92% 救援)、事件型5/6反弹, 唯独混合型3/4扛到 -52%(样本小, 用户接受风险)。改2处: ① `_evaluate_position` (b) 段 `_direct_stop=(tier=="hybrid")`, `if _reeval_on and not _direct_stop` 才 PENDING_REEVAL, 否则走硬卖 (reason 带"混合型到值直接平仓"); ② `_maybe_trigger_auto_reeval` 顶部 `if tier=="hybrid": return` (不起自动重评)。启动日志 + tier 下拉说明同步。自测路由: 混合→直接卖 / 收敛+事件→重评, _maybe_trigger 混合跳过。
  - `7.4.6` (2026-07-08, patch, 真钱数据修: **录入入场价一定用"当次买的"**): 用户发现 US-Iran(二次入场: 之前 $0.71 平过、这次 $0.52 买回)录入时用了旧入场价 $0.71。根因: `/api/record_position` 用前端传的 `entry_price`(= 持仓行 `data-avg`, 页面没刷新/二次入场时是上一轮旧价), 只在 ≤0 才从 Polymarket 现价兜底。改: **record_position 永远实时拉 Polymarket 当前 `avg_price` 覆盖前端值**(拉不到才退回前端值, 都没有才报错), 差值 >0.001 时 log 记录覆盖。`save_position_meta` 只此一路写 entry_price → cjApplyPos / 手动表单 / saveTP 全覆盖。不改策略(PPT 只跟版本号)。
  - `7.5.0` (2026-07-08, **minor** = ⚡自动录入): JSON 快速通道新增「⚡ 存入自动录入」— 推荐存服务器 `pending_recs` 表(24h 过期, 同 slug+side 新顶旧=superseded), monitor 心跳 `_auto_record_from_pending` 把**新出现的无 meta 仓位**按 slug+side 配对 → 配上自动写全套 meta(q/信心/止损档/cluster/tag, entry=实时 avg, 与手动录入同口径)+标 applied(前端草稿同步移除); 配不上/q≤成本 → `app_state.auto_record_alerts` 主页琥珀闪窗提醒人工(粘性: 补录或仓位消失才撤)。已手动录过的推荐自动标 superseded; `executor.get_positions` 补 slug/endDate 字段; 新路由 `/api/pending_recs`(GET/POST)+`/clear`。🔒 只写元数据永不下单、绝不覆盖已有 meta; 8 场景假数据端到端测试通过。详见下方「v7.5 新增的关键设计」。
  - `7.5.1` (2026-07-08, patch, **自动录入匹配加固**): 实测第一单就配不上 — Claude 给的 slug 是烂的 (`...byptptpt-旧时间戳`, 市场根本不存在)。修: ① 存入时 `_resolve_rec_market` (dashboard.py) 把 slug 解析成真实市场 — Gamma 精确查 → public-search 模糊搜 (归一化去尾部时间戳、公共前缀打分 ≥15字符且≥60%, `end_date` 决胜 by-july-15/31 这类兄弟市场), `pending_recs` 加 token_yes/token_no/resolved_slug/resolved_title 4 列, 存入 toast 显示解析到的市场标题 (烂 slug 当场看得见); ② monitor 匹配改**三层阶梯**: token 精确 → slug/resolved_slug 精确 → 归一化前缀模糊 (对实际仓位 slug 打分, **唯一赢家才自动填**, 平手/方向不一致转人工提醒); ③ 已录过查重同样认 resolved_slug/token。伊朗油轮仓实测: 老烂 slug 推荐经 fuzzy 层自动配上 (`via fuzzy`), 解析路径 (`via token`) 也验证通过。
  - `7.5.2` (2026-07-09, patch, **GLM 重评被 token 上限截断 → 给足预算让它写完**): 用户发现最新 GLM 自动重评"没返回结果"(id=29 俄乌题)。根因: GLM `reasoning_effort=max` 思考极长, `max_tokens=32000` 在吐 JSON 前就被 `finish_reason=length` 截断 → `_parse_glm_decision` 找不到决策 → "GLM 未返回 JSON 决策"(**不是超时**: GLM 有回应, 编排 `t.join()` 无超时, client 600s 也没到)。用户要保持 max 思考、让它写完。改 `auto_reeval.py`: ① `GLM_MAX_TOKENS` 32000→**120000**(贴近 glm-5.2 128K 上限, env `AUTO_REEVAL_GLM_MAX_TOKENS`); ② client 超时 600→**1200s**(新常量 `GLM_TIMEOUT`, 仍 <30min expire_stale); ③ `_run_glm` 加 `finish_reason`+`completion_tokens` 日志(下次一眼看出 length 截断 vs stop 正常完成), length 时 log.warning; ④ 失败存 raw_text 1000→4000 便于调试。实测真跑一条(伊朗油轮仓): finish_reason=stop / 8447-of-120000 token / 265s / 出 update_q(0.7) ✅。GLM 是备用+对比模型, Claude 权威不受影响。
  - `7.5.3` (2026-07-10, patch, 真钱: **事件型禁止 API 取消止损**): 用户指出 `cancel_autostop` 用在事件型上自相矛盾 —— 7.4.3 特意给事件型加了 -60% 兜底("不再只靠地板一路跌到底"), 而 cancel_autostop 正好把这道 -60% 撤掉、退回只剩地板; 且事件型已有 exit 护栏保护, AI 想扛用 hold/update_q 就够。新增 `guard_event_driven_cancel`(纯函数, 平行于 `guard_event_driven_exit`): 事件型 + cancel_autostop → 降级 `update_q`(有合法 new_q)或 `hold`。两处 API 执行路径都加(`_auto_execute` 离线 + `dashboard.auto_reeval_confirm` 在线确认)+ 重评提示词 4 处写明"事件型禁用 cancel_autostop"。⚠️ **只挡 API 重评; 用户手动 `/api/toggle_auto_stop`(🛑止损OFF 按钮)照常能关那道 -60%**。收敛型 cancel_autostop 不受影响。自测 5 例全对。
  - `7.5.4` (2026-07-13, patch, **交易复盘报告 + `/report` 网页**): 用户要一份 5月中旬→现在的复盘, 能在其他设备看。① `scripts/gen_review_pdf.py`(reportlab, 中文 STSong-Light CID 字体, 横版)生成 11 页 PDF: 总览/分期/标签/**亏损归因(只看是否赚钱: 卖飞55%/出场方式/入场价favorite-longshot/tier)**/赚vs亏对比/**卖飞清单16笔**/结论/**反锚定验证(价格大跌q跟不跟着跌: 价跌39%时q只降14pp、相关+0.05、new_q贴大跌前中枢52%而非坑底36%→反锚定有效)**/多次重评q轨迹/每笔明细。② **`/report` 路由**(dashboard.py + `_REPORT_HTML` 常量)= **实时从 DB 渲染的 HTML 报告**(手机友好深色响应式, 不依赖任何文件 — PDF 被 gitignore+清理删掉过, 所以走 DB), 经 Tailscale `https://<tailnet>/report` 密码鉴权后任何设备可看。PDF 版 gitignore(含真实盈亏)。⚠️ 铁律: 用 `get_conn`+`row_factory=Row` 实时查, 别缓存文件。

## v7.5 新增的关键设计 (不要随意改回)

- **⚡自动录入 (2026-07-08 用户要求"不想每次点录入, 存一天的 JSON 自动配")**: 主页 JSON 通道「⚡ 存入自动录入」→ `pending_recs` 表 (状态 pending/applied/superseded/expired/cleared; 24h 过期 `expire_pending_recs`; 同 slug+side 新顶旧; 清空=cleared 不删) → monitor 心跳 `_auto_record_from_pending(positions)`: **新出现的无 meta 仓位**按 slug+side 配对待录推荐 → 配上写全套 meta (q/信心/止损档/cluster/tag, entry=实时 avg_price, 与 /api/record_position 同口径) + 标 applied + `log_event("AUTO_RECORD",...)`; 配不上 (仅待命中才产生新提醒) / q≤成本 (TP 方向错) → `app_state.auto_record_alerts` → 主页**琥珀色**闪窗 `#autorec-pop` (跟红色重评闪窗分开; **粘性**: 直到补录 meta 或仓位消失才撤, 待录清空也不撤)。已手动录过的推荐自动标 superseded。**🔒 三条铁律: 只写元数据永不下单/卖出; 绝不覆盖已有 meta; q≤成本不自动填、转人工。**
- 前端: `cjSaveAuto`(存) / `cjAutoLoad`(20s 轮询: 待命·已自动录·待人工 chips + 已录的自动从浏览器草稿移除, 同手动录入成功行为) / `cjAutoClear`(停用清空) / `arcDismiss`(闪窗"知道了", 同批内容不再弹)。路由 `/api/pending_recs` GET/POST + `/api/pending_recs/clear`。
- **`executor.get_positions()` 补了 `slug`/`end_date` 两个映射字段** (data-api 本来就返回, 之前没接) — 自动配对靠它, **别删**。
- **匹配 = 三层阶梯 (v7.5.1, 因 Claude 常给烂/旧 slug)**: ① token 精确 (存入时 `_resolve_rec_market` 已把 slug 解析成真实市场, 存了 token_yes/token_no) ② slug/resolved_slug 精确 ③ 归一化前缀模糊 (`_norm_slug` 去尾部时间戳; ≥15字符且≥60%公共前缀; **唯一赢家才自动填, 平手转人工**; token 对上但方向不一致 → 绝不填, 提醒"买反了")。解析失败的存入时 toast 就警告, 仍靠 ③ 兜底。**别把模糊层的"唯一赢家"约束去掉** — 那是防错填元数据(会驱动卖出决策)的关键闸。
...
- 公网 URL: https://baymaxs-mac-mini.tailc6dc26.ts.net (Tailscale Serve, tailnet-only, v5.7)
- 数据库: v4.db (SQLite), 共用 venv 在 ../polymarket-bot/.venv
- v3 在 ~/polymarket-semi-auto (frozen, 不要碰)
...
- 不修改 v3 代码 (~/polymarket-semi-auto)
