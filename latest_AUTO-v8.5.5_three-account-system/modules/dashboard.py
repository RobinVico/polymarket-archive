...
<select id="tier-{{ loop.index0 }}" class="conf-sel" title="止损分级 (Claude 入场分类决定): 收敛 回撤20%(≤3天12%)+确认→交重评 / 混合 回撤35%+确认→直接平仓(v7.4.5, 不走重评) / 事件 -50%强行止损直接平仓(AUTO 2026-07-18, 任何东西越不过)+$0.05地板 (必须选一档, 无未分类)" onchange="saveTier('{{ p.asset }}', this.value)">
...
def create_app():
    ...
    def auto_reeval_confirm():
        ...
        token_id = s["token_id"]
        action = s.get("action")
        # AUTO 分叉 (2026-07-18 用户最高指令): 事件型 exit 护栏已删 (原需论点破或 edge≤-8pp 才放行) —
        #   确认路径也一样, action 一是 exit 就执行, 不再降级。方向纠错闸在决策生成阶段, 这里不重复。
        try:
            if action == "exit":
                exe = Executor.get()
                pos = next((p for p in exe.get_positions() if p.get("asset") == token_id), None)
    ...
    # AUTO 分叉 (用户 2026-07-08 铁律: tags 不能变): /tags 动态标签页 + 采纳/拉黑路由已整体删除。
    return app
