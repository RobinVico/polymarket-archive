...
3. **你的估算**: xx% (基于 Bull/Bear 证据的独立判断; 直接填你真实认为的概率, **不做任何往市场价靠拢/收缩的调整 —— 原始判断是多少就填多少**)
4. **Edge**: 你的估算 - 当前价 (个百分点)
5. **执行成本**: 看扫描报告里每个市场标注的 "$5 taker 滑点 X pp"。
   总执行成本 = taker 滑点 + 双边手续费 (约 2.2pp). 不知道滑点时按 3pp 估算。
6. **实际可获取 Edge** = Edge - 执行成本
   门槛比的是 **Edge** (见下面 §推荐门槛); 但务必同时确认扣掉执行成本后**净 edge 仍为正** —— 这就是 5pp 硬地板的来由。
   例: edge 看似 10pp, 如果执行成本 5pp, 实际净 edge 只剩 5pp, 已贴地板, 谨慎。
...
## 算法 (对每个候选)
**最终门槛 = max(5, 基础门槛 + 价位叠加)。edge ≥ 最终门槛 才推荐。**
...
> **原始估算**: xx%
> **估算**: xx%  ← 这个数字填进 Dashboard 的 TP 输入框
> **Edge**: xx 个百分点
...
- `q` = 估算, `cur_price` = 你下注方向 token 的当前价 — **都用 0-1 小数** (0.84, 不是 84%).
...
- slug 原样复制,不要改。
- "当前价"和"估算"必须填同一个方向的token价格。比如买NO,两者都填NO价。
- 承认不确定比假装确定好。
- edge < 最终门槛(基础 6/8/10pp ± 价位叠加, 见 §推荐门槛)就说无推荐, 不要凑。
- 实际可获取 edge (edge - 执行成本) < 5pp 也说无推荐 (净 edge 硬地板).
...
# 年化 IRR 评估 (v4.1 必填)
推荐每个标的时**必须计算年化 IRR**:
  年化 IRR = (你的估算 - 当前价) / 当前价 × (365 / 结算天数) × 100%
...
例: 估算16%, 当前 4%, 结算 75 天 → IRR = (16-4)/4 × 365/75 × 100 = 1460% → 优先推荐
例: 估算60%, 当前 50%, 结算 60 天 → IRR = 10/50 × 365/60 × 100 = 122% → 可推荐
例: 估算60%, 当前 50%, 结算 250 天 → IRR = 10/50 × 365/250 × 100 = 29% → 不推荐
...
- A4. 关键解析机构"非事件"检查
- A5. 给出你的估算 q + confidence + 区间 (直接填你真实判断, **不做任何往市场价靠拢/收缩的调整**)
- A6. 算 Edge 和实际可获取 Edge

阶段 B 内部必须完成:
- B1. 24h 新事件列表 (附 URL + 时间戳)
- B2. q_today = q ± Δ (在 A 的估算基础上按 24h 新事件微调; 单日调整 > 5pp 需 ≥ 2 条 URL)
- B3. 反向论证 (24h 新闻持久性 + 是否已被市价消化)
...
4. q_today = 你综合调研后的最终原始判断, 就是决策依据 (**不做任何往市价的校准/收缩, 原始判断多少就是多少**)
5. Edge 看实际可获取 (扣滑点+手续费)
...
Edge: {{±A}}pp | 执行成本 {{Y}}pp | **实际可获取 {{±B}}pp**
...
def build_reeval_prompt(meta, cur_price, days_to_resolution, progress_pct=None, pre_dump_center=None):
    ...
    raw_estimate_line = f"\n你之前估的 q: {raw_est*100:.1f}%" if raw_est else ""
