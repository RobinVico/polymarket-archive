...
def _resolve_tag_cfg(tag_label):
    """tag 配置: 只查 tags.py 固定白名单 TAGS。
    AUTO 分叉 (用户 2026-07-08 铁律: tags 不能变): 动态采纳标签功能已整体删除。"""
    from modules.tags import TAGS
    return TAGS.get(tag_label)
...
def scan_by_tag(tag_label, mode="standard", **kwargs):
    ...
    for ev in events:
        ...
        for m in ev_markets:
            # AUTO 分叉 (2026-07-12 用户拍板, 原"黑名单永远排除"改"永远只进测试仓"):
            # 黑名单市场不再 continue 丢弃 —— 保留进报告照样喂 GLM, 只打标记; 命中即由 auto_trader
            # 无视价格强制只进测试仓 (绝不真买)。白名单 tag 上下文里 is_blacklisted 为真 = 关键词命中
            # 标题/slug (白名单已覆盖 tag 级黑名单), 正是要限制的目标集。
            _blacklisted = bool(is_blacklisted(m, ev))
            # 提取 outcomePrices
            prices = m.get("outcomePrices", "")
            if isinstance(prices, str) and prices:
                try:
            ...
            mkt_obj = {
                "question": m.get("question", ""),
                "slug": m.get("slug", ""),
                "blacklisted": _blacklisted,   # AUTO 2026-07-12: 命中黑名单关键词 → 报告打标记 → 只进测试仓
                "yes_price": yes_price,
                "no_price": float(prices[1]),
...
def generate_tag_report(tag_label, tag_cfg, events_dict, stats, cfg):
    ...
    from modules.tags import get_tag_hint, BLACKLIST_REPORT_MARK
    ...
    for i, (ev_id, group) in enumerate(events_dict.items(), 1):
        ...
        for j, m in enumerate(group["markets"], 1):
            ...
            # AUTO 2026-07-12: 黑名单市场在 slug 行尾打标记 (下游 auto_trader 强制只进测试仓 /
            # auto_discovery 数候选门槛时扣掉它); 宇宙闸门认的 `slug` 反引号子串不受影响。
            _bl_mark = ("  " + BLACKLIST_REPORT_MARK) if m.get("blacklisted") else ""
            lines.append(f"- Slug: `{m['slug']}`{_bl_mark}")
            lines.append(f"- YES价格: {m['yes_price']:.1%}  |  NO价格: {m['no_price']:.1%}")
            lines.append(f"- 最低卖价: ${m.get('best_ask',0):.3f}  |  最高买价: ${m.get('best_bid',0):.3f}")
            rel_sp = m.get('rel_spread', 0) or 0
            slippage = m.get('taker_slippage_pp', 0) or 0
...
def scan_all_tags(tier_filter=(1, 2), mode="standard", tag_workers=5):
    ...
    # AUTO 分叉 (用户 2026-07-08 铁律: tags 不能变): 动态标签合并已删除, 只扫 tags.py 白名单。
