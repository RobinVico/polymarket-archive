...
# Load credentials from local .env first; user can override path via POLY_ENV_PATH env var.
# Historically this project shared its .env with a sibling v3 codebase; the path is now
# configurable so the public release doesn't hard-code a personal directory.
_extra_env = os.environ.get("POLY_ENV_PATH")
if _extra_env:
    load_dotenv(_extra_env)
load_dotenv()  # also load .env in current working dir (CWD where main.py runs)
...
# AUTO 分叉补丁 (2026-07-08): 按 token 自动查 negRisk + tick_size (下单参数), 进程内缓存。
# 背景: 老代码写死 tick_size="0.01" 且不传 neg_risk → 多结果(negRisk)市场/0.001-tick 市场下单必败。
_MARKET_OPT_CACHE = {}
...
class Executor:
    ...
    def get_positions(self):
        try:
            ...
            Executor._live_positions = (time.time(), positions)   # v7.1 提速: 心跳缓存, 供只读轮询接口免重复联网
            return positions
        except Exception as e:
            log.warning(f"get_positions failed: {e}")
            return []

    def get_positions_cached(self, max_age=35):
        """v7.1 提速: 返回心跳(每30s)缓存的持仓, 新鲜就直接给 (不联网); 过期/无缓存则现拉。
        给 dashboard 只读轮询接口用 —— 反正数据只反映心跳节奏, 没必要每次 poll 都打 data-api (timeout 30s)。"""
        c = getattr(Executor, "_live_positions", None)
    ...
    def get_cash_balance(self):
        ...
        try:
            ...
            if bal_raw is None:
                return None
            bal = float(bal_raw) / 1_000_000
            Executor._live_cash = (time.time(), bal)   # v7.1 提速: 心跳缓存
            return bal
        except Exception as e:
            log.warning(f"get_cash_balance failed: {e}")
            return None

    def get_cash_cached(self, max_age=35):
        """v7.1 提速: 返回缓存的现金余额 (新鲜直接给), 过期/无则现拉。"""
        c = getattr(Executor, "_live_cash", None)
        if c and (time.time() - c[0]) < max_age:
            return c[1]
    ...
    def buy(self, token_id, usdc_amount, reason=""):
        ...
        try:
            ...
            # AUTO 分叉补丁 (2026-07-08, sig=3 存款钱包流): 新 CLOB 要求市价买单的 maker 金额
            # (=size×price) 最多 2 位小数。整数股数只对 2 位小数价格够用; 0.001-tick 市场 (如 0.674)
            # 还要凑步长: size 必须是 step 的倍数, step = 10/gcd(价格千分位, 10) ∈ {1,2,5,10}。
            price_th = int(round(best_ask * 1000))
            step = 10 // math.gcd(price_th, 10)
            size = float(int(usdc_amount / best_ask))
            size -= size % step
            while size * best_ask < 1.0:          # 保证 ≥$1 最小单
                size += step
            if size * best_ask > usdc_amount + max(2.0, usdc_amount * 0.75):
                return False, f"步长{step}股凑不出接近 ${usdc_amount} 的金额 (ask={best_ask})"
            if size < 1:
            ...
            try:
                result = self.client.create_and_post_order(
                    ...
                    options=_order_opts(token_id),   # AUTO: 自动带 negRisk + 真实 tick_size
            ...
            # AUTO 修正: v2 市价买单 makingAmount=花掉的USDC, takingAmount=股数 (老代码标反了)
            try:
                shares = float(result.get("takingAmount", 0) or 0)
            except (ValueError, TypeError):
                shares = 0.0
            usd_spent = making_amount
    ...
    def sell(self, token_id, size, reason=""):
        ...
        try:
            ...
            if making_amount < size * 0.95:
                # v5.7 (P1): partial fill < 95% returns False so caller does NOT clear position_meta.
                # The making_amount portion is genuinely sold on Polymarket; the remainder stays as
                # a live position. Next monitor heartbeat will see the reduced size and re-evaluate
                # whether to keep trying (止盈/止损 thresholds reapplied on new size).
                # Previously (v5.6 and earlier) we returned True here, causing monitor to clear meta
                # while Polymarket still held a dust position — silent money loss.
                log.warning(f"sell部分成交 ({pct:.0f}%, 请求{size} 实际{making_amount:.4f}股 order_id={order_id}). 视为失败, monitor 下轮重试剩余仓位")
                return False
