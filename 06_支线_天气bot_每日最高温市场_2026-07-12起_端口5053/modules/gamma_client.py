"""Gamma API HTTP 客户端, 带 DNS 污染兜底. (copy 自主项目 modules/gamma_client.py v5.10.2)
...
# === 进程级 DNS 兜底 (覆盖 requests / urllib3 / httpx / CLOB SDK 全部) ===
...
def _pinned_get(host, path, params, timeout):
    ...
    for ip in _doh_resolve(host):
        try:
            ...
            r = pool.request("GET", url, headers={"Host": host, "User-Agent": "tianqi-bot/0.1"})
            if r.status == 200:
